...
if [ "$darwin" = "true" -a "$USE_JAVA_MODULES" = false ] ; then
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    OURCYGPATTERN="(^($ROOTDIRS)$|^/opt/gradle/|^/pro)"
    OURCYGPATTERN="(^($ROOTDIRS)$|^/opt/gradle/|^/pro)"
...
# Escaping properties for Groovy now to avoid wrong words to be left unescaped in ant calls
